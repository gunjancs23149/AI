document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchRecommendations() {
        ...
        if (!userId) {
            showToast("Please enter a User ID to begin.");
            return;
        }
        ...
        getRecBtn.innerHTML = `<span>Analyzing...</span>`;
        ...
        try {
            ...
            getRecBtn.innerHTML = `<span>Discover</span><svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>`;
    ...
    function renderRecommendations(recommendations) {
        ...
        recommendations.forEach((product, index) => {
            ...
            const productKeywords = {
                ...
                'P11': 'desklamp',
                'P12': 'smartwatch',
                'P13': 'bluetooth,speaker',
                'P14': 'blanket',
                'P15': 'cookware'
            };
            
            const keyword = productKeywords[product.product_id] || 'product';
            // Use loremflickr for beautiful categorized and specific images, lock with product ID to keep it consistent
            const dummyImageUrl = `https://loremflickr.com/600/400/${keyword}?lock=${product.product_id.replace('P', '')}`;

            const card = document.createElement('div');
            card.className = 'card-clean';
            // Stagger animation based on index
            card.style.animation = `fadeUp 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards ${index * 0.08}s`;
            ...
            card.innerHTML = `
                ...
                <div class="card-body">
                    ...
                    <div class="card-rating">
                        ${starsHTML} <span class="rating-num">(${product.rating.toFixed(1)})</span>
                    </div>
                </div>
            `;
